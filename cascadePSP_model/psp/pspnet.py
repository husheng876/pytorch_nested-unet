...
from cascadePSP_model.psp import extractors
from cascadePSP_model.sync_batchnorm import SynchronizedBatchNorm2d
...
class PSPModule(nn.Module):
    ...
    def forward(self, feats):
        ...
        priors = set_priors + [feats]   #两个列表合并
        bottle = self.bottleneck(torch.cat(priors, 1))  #在指定维度连接给定序列的张量
...
class PSPUpsample(nn.Module):
    def __init__(self, x_channels, in_channels, out_channels):
        ...
        self.conv = nn.Sequential(
            SynchronizedBatchNorm2d(in_channels),   #多gpu正则化
...
class PSPNet(nn.Module):
    def __init__(self, sizes=(1, 2, 3, 6), psp_size=2048, deep_features_size=1024, backend='resnet34',
                 pretrained=True):
        super().__init__()
        self.feats = getattr(extractors, backend)(pretrained)#返回对象属性值
    ...
    def forward(self, x, seg, inter_s8=None, inter_s4=None):
        ...
        if inter_s8 is None:
            ...
            f, f_1, f_2 = self.feats(p)    #对于ResNet的三个返回值
            p = self.psp(f)
