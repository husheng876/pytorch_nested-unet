...
class RDC(nn.Module):
    def __init__(self, hidden_dim, kernel_size, bias, decoder='GRU'):
        ...
        self.padding = kernel_size // 2, kernel_size // 2   #整除，为何padding的形式是这样的，是否需要更改成别的样子
        self.bias = bias
        self.decoder = decoder
        self.gru_catconv = nn.Conv2d(self.hidden_dim * 2, self.hidden_dim * 2, self.kernel_size,
                                      padding=self.padding, bias=self.bias)#param1，2:input channels,output channels，数据形式是默认channel first
...
class UNetRNN(nn.Module):
    ...
    def forward(self, input, cell_state=None):
        ...
        h0 = self._init_cell_state(x1)  # 1/16,512       返回与x1大小相同的零张量
